...
your `browser_call.reason`. Always provide a clear reason instead of guessing
undocumented params.
...
- Interaction targets come from the latest `DOM.getAXTree` or a verified
  ...
  link such as `Pros & Cons`, extracts the live `[uuid:n:n]` canonical id, and
...
state. After navigation, verify URL/title with `Page.getState` and refresh page
structure before extracting.

Avoid tight `Page.getState` loops that merely hope for change. Act on returned
feedback, notifications, or a fresh semantic perception after a state-changing
Input action.
...
AXTree ids are physical anchors returned by the live page, for example a value
shaped like `38c7a3b8-0000-4000-9000-c5e1b77d3193:42:0`. Use the actual id
...
selector form). Do not copy this example id.
...
Use Input capabilities for user-like actions. Prefer current AXTree ids or
stable selectors. After click, type, press, or scroll, re-perceive with
`DOM.getAXTree` or verify with `DOM.getAttribute`/`DOM.getText` before
continuing.
...
for `timeout` or `page_settled_after_hitl`, finalize with a blocker.
