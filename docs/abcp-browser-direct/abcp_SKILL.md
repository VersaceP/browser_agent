...
- `Input.scroll` is a three-mode union with no top-level locator: `target:{id?,selector?}` (no `direction`; success is `targetVisible=true`), `container:{id?,selector?}` with `direction`/`amount` for an already-visible container, or neither for the page viewport. `amount:0` reads state and is container/viewport only. Use `layers[].delta` for real movement; do not repeat a direction after `completedReason="boundary-reached"`.
- `Input.select` takes the select control locator (`id` and/or `selector` - at least one, both allowed, id resolved first) plus the selection payload in the shape the connected build's `System.describeAction` schema declares for `Input.select`, copied verbatim from a fresh `DOM.inspectSelect`. That shape is contract-generation specific — some builds take one mutually exclusive array (`nativeValues` for a native HTML `<select>`, or `optionIds`/`optionLabels` for the open custom popup), others take the `selections` envelope of per-option `{value|id|label}` objects — so read the live schema for this connection instead of assuming either, and hold the entries to exactly the item constraints that schema states — no more (a build whose item type is a bare `string` accepts an empty native value) and no less. `DOM.inspectSelect` reads only the CURRENT option window (it never searches/scrolls/paginates): when `optionWindow.coverage.kind` is `current-window`, the window is incomplete and `Input.select` will NOT explore it - keep the popup open, combine `DOM.getSemanticTree` via `popup.id` with ONE generic `Input.type`/`Input.scroll`/`Input.click` per step, and re-inspect after every mutation. On `select-agent-takeover-required` (or popup codes like `select-popup-not-ready`/`select-popup-relation-changed`), do NOT replay `Input.select`; continue with one bounded generic action after re-observing. Option ids are valid only for the popup generation that exposed them; if inspect reports `select-target-not-select`, traverse the UI with fresh AXTree targets plus one verified `Input.click` per visible level instead.
- Use `Runtime.evaluate` only after every available Page/AX/Semantic/text/attribute read was actually attempted in the current page epoch. It is read-only and model-authored calls must explicitly use `world=isolated`. Only `reason_kind=non_dom_state` may authorize one harness-controlled strict main retry after `ReferenceError("ABCP_MAIN_WORLD_REQUIRED:<global>")`; ordinary errors, timeouts, and empty values do not. Never request main/auto directly, mutate page state, bypass permissions, or replace form interactions.
