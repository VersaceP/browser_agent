...
- Every `Input.select` selections item carries EXACTLY ONE locator: `id`, exact `value`, exact `label`, or `path` (exclusive, and each segment follows the same rule). Multiple direct choices declare the final selection set on a confirmed multi-select control, not an append.
