...
- Use `Runtime.evaluate` only after every available Page/AX/Semantic/text/attribute read was actually attempted in the current page epoch; `Page.getState` is not one of those reads. Keep it read-only. `world` is a required parameter: `auto` lets ABCP prefer the isolated world and retry a context-mismatch failure in the main world, while `isolated` and `main` are strict - choose `isolated` when the expression must not see page globals, and `auto` when it may need them. Never mutate page state, bypass permissions, or replace Input/File interactions with script.
