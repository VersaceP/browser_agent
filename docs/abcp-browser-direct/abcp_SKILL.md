...
- `Input.select` takes the select control locator (`id` and/or `selector` - at least one, both allowed, id resolved first) plus EXACTLY ONE selection array copied verbatim from a fresh `DOM.inspectSelect`: `nativeValues` for a native HTML `<select>`, or `optionIds`/`optionLabels` for options in the currently open custom popup. The three arrays are mutually exclusive, non-empty, and duplicate-free; the old `selections` envelope and cascade `path`s are gone. `DOM.inspectSelect` reads only the CURRENT option window (it never searches/scrolls/paginates): when `optionWindow.coverage.kind` is `current-window`, the window is incomplete and `Input.select` will NOT explore it - keep the popup open, combine `DOM.getSemanticTree` via `popup.id` with ONE generic `Input.type`/`Input.scroll`/`Input.click` per step, and re-inspect after every mutation. On `select-agent-takeover-required` (or popup codes like `select-popup-not-ready`/`select-popup-relation-changed`), do NOT replay `Input.select`; continue with one bounded generic action after re-observing. Option ids are valid only for the popup generation that exposed them; if inspect reports `select-target-not-select`, traverse the UI with fresh AXTree targets plus one verified `Input.click` per visible level instead.
