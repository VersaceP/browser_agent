...
# ``find_in_axtree`` still supports regex over an accessible name/text query.
# Python's stdlib ``re`` cannot interrupt a pathological match, so keep this
# deliberately conservative boundary even though the larger whole-line grep
# feature is gone.  Safe historical patterns are short alternations/anchors;
# quantified groups and backreferences are unnecessary for this tool.
_AXTREE_NAME_REGEX_MAX_CHARS = 200
_AXTREE_QUANTIFIED_GROUP_RE = re.compile(
    r"\)\s*(?:[*+?]|\{\s*\d*\s*,)"
)
_AXTREE_BACKREFERENCE_RE = re.compile(
    r"(?:\\[1-9]|\\g<|\(\?P=)"
)
_AXTREE_UNBOUNDED_QUANTIFIER_RE = re.compile(
    r"(?<!\\)(?:[*+]|\{\s*\d*\s*,\s*\})"
)
_AXTREE_UNBOUNDED_QUANTIFIER_LIMIT = 2


def _reject_unsafe_axtree_name_regex(pattern: str) -> Optional[str]:
    """Return a rejection reason for regex that stdlib ``re`` cannot bound.

    This is a small safety boundary for the retained name/text regex mode, not
    a whole-line query language or a general static regex analyser.  It accepts
    the simple anchors and alternations seen in historical calls while denying
    the constructs that can make one model-authored search hang the worker.
    """
    if len(pattern) > _AXTREE_NAME_REGEX_MAX_CHARS:
        return (
            f"pattern is {len(pattern)} chars; limit is"
            f" {_AXTREE_NAME_REGEX_MAX_CHARS}"
        )
    if _AXTREE_BACKREFERENCE_RE.search(pattern):
        return "backreferences are not allowed"

    # Escaped punctuation is literal, so remove it before looking for regex
    # operators.  This intentionally errs on the conservative side.
    stripped = re.sub(r"\\.", "", pattern)
    if _AXTREE_QUANTIFIED_GROUP_RE.search(stripped):
        return "quantifiers applied to groups are not allowed"
    unbounded = len(_AXTREE_UNBOUNDED_QUANTIFIER_RE.findall(stripped))
    if unbounded > _AXTREE_UNBOUNDED_QUANTIFIER_LIMIT:
        return (
            f"{unbounded} unbounded quantifiers; limit is"
            f" {_AXTREE_UNBOUNDED_QUANTIFIER_LIMIT}"
        )
    return None
...
def _find_in_axtree(agent: Any, tool_input: JsonDict) -> JsonDict:
    ...
    # Both fields are required by the strict tool schema so checking only for
    # ``is not None`` makes ``text`` unreachable whenever callers pass the
    # conventional ``name: ""``.  Preserve the historical alias/fallback
    # contract without bringing back whole-line grep semantics.
    query = str(
    ...
    if match_mode == "regex" and query:
        rejection = _reject_unsafe_axtree_name_regex(query)
        if rejection:
            return {
                "status": "failed",
                "error": f"unsafe name/text regex: {rejection}",
            }
