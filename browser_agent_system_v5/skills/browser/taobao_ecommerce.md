...
**⚡ 直达导航（极其重要！优先级最高！）**：
如果任务描述中已提供商品详情页 URL（如 `https://item.taobao.com/item.htm?id=xxx`），
**必须直接 navigate 到该 URL**，禁止先打开淘宝首页再搜索进入！直接导航可以节省大量轮次和 Token。

```
1. 直接 navigate 到商品详情页 URL（如果任务已提供）
   - 如任务提供了 URL → 直接 navigate(url="已提供的URL", wait=5)
   - 如任务未提供 URL → 通过搜索 URL 找到商品后再进入
2. 等待页面加载（wait=3-5）
3. 检查是否需要登录
4. 提取基本信息（标题、价格、店铺）
5. 如需评论，滚动到评论区
