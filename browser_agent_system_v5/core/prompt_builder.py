...
def build_dynamic_context(task: str, worktree_path: str = "", env_vars: Optional[Dict[str, str]] = None, skills: str = "") -> str:
    """
...
def format_skills(skills: List[Any]) -> str:
    ...
    将技能列表格式化为带有清晰边界标记的字符串，用于注入到动态上下文中。
    边界标记用于防止提示词注入攻击。
    
    :param skills: 技能对象列表（List[Skill]）
    :return: 格式化后的技能内容字符串
    ...
    for idx, skill in enumerate(skills, 1):
        # 添加技能标题（带边界标记）
        parts.append(f"【技能 {idx}: {skill.name} v{skill.version}】")
        # 添加技能内容
